...
4. **Clean workspace before starting**: Delete any previous episode dirs in `/tmp/podcast-clips/` and any deployed clips in `app/static/clips/` that are not the current target. Old test episodes (ep1, ai-tutorial, etc.) left in the workspace produce wrong captions on subsequent runs — verify with `ls /tmp/podcast-clips/` and `ls app/static/clips/` before starting.
